...
Verifies:
  ...
  - the embedded hedge blocks match a fresh build_hedge_section(hedge_blocks_2026.json)
"""
import json
import os
import re
...
HEDGE_SOURCE_PATH = os.path.join(HERE, "hedge_blocks_2026.json")
...
def main():
    ...
    raw_hedge_rows = load_json(HEDGE_SOURCE_PATH)
    expected_hedge = build_hedge_section(raw_hedge_rows)
    assert dataset["hedge"] == expected_hedge, \
        "embedded hedge data does not match hedge_blocks_2026.json"
